...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if RWKV_HEAD_QK_DIM > 0:
            ...
            #x = self.head(mean_pooled)
            x = self.classifier(mean_pooled)

        loss = None
        if targets is not None:
            loss = F.cross_entropy(x.view(-1, x.size(-1)), targets.to(x.device).view(-1))
            # Token-level perplexity calculation (assuming input_ids is available)
            shift_logits = x[:, :-1, :]
            shift_labels = targets.to(x.device)[:, 1:]
            log_probs = F.log_softmax(shift_logits, dim=-1)
            per_token_ppl = torch.exp(-log_probs.gather(-1, shift_labels.unsqueeze(-1)).squeeze(-1))
            #print(per_token_ppl)
