...
from src.out_model import GPT, GPTConfig
import torch
import datasets
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        pred = torch.argmax(last_logits[:, :2], dim=-1)
    ...
    for batch in tqdm(test_loader):
        ...
        total_correct += (torch.argmax(last_logits[:, :2], dim=-1) == labels).sum().item()
    ...
    torch.save(model.state_dict(), "sst_model.pth")
rec_file.close()
